...
#include "dhcp/includes/dhcpd.h"
#include "dhcp/includes/omapip/omapip_p.h"
#include <syslog.h>
#include <signal.h>
#include <errno.h>
#include <limits.h>
#include <sys/types.h>
#include <sys/time.h>
#include "dhcp/bind/include/isc/file.h"
...
const char *path_dhcpd_db = "dhcpd.leases";
const char *path_dhcpd_pid = "dhcpd.pid";
...
static void omapi_listener_start(void *foo)
{
    ...
    if (result == ISC_R_SUCCESS && omapi_key)
        result = omapi_protocol_configure_security(listener, verify_addr, verify_auth);
    if (result != ISC_R_SUCCESS)
    {
    ...
    char *interface_to_bind_to)
{
    ...
    path_dhcpd_conf = config_file;
    have_dhcpd_conf = 1;

    struct interface_info *tmp =
        (struct interface_info *)0;
    if (strlen(interface_to_bind_to) >= sizeof(tmp->name))
        log_fatal("%s: interface name too long "
                  "(is %ld)",
                  interface_to_bind_to, (long)strlen(interface_to_bind_to));
    result = interface_allocate(&tmp, MDL);
    if (result != ISC_R_SUCCESS)
        log_fatal("Insufficient memory to %s %s: %s",
                  "record interface", interface_to_bind_to,
                  isc_result_totext(result));
    strcpy(tmp->name, interface_to_bind_to);
    if (interfaces)
    {
    ...
    if (local_family == AF_INET6)
    {
        ...
        if (!have_dhcpd_db)
        {
            if ((s = getenv("PATH_DHCPD6_DB")))
                ...
                path_dhcpd_db = "/db/dhcpd6.leases";
        }
        if (!have_dhcpd_pid)
        {
...
{
    char *config_file = "/etc/dhcp/dhcpd.conf";
    char *interface_to_bind_to = "edge0";

    return main_extracted(config_file, interface_to_bind_to);
}
...
void lease_pinged(from, packet, length) struct iaddr from;
u_int8_t *packet;
int length;
{
    struct lease *lp;
...
void lease_ping_timeout(vlp) void *vlp;
{
    struct lease *lp = vlp;
